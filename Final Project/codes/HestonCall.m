function C=HestonCall(St,K,r,sig,T,vt,kap,th,lda,rho)
%--------------------------------------------------------------------------
...
f1 = CF_Heston(log(St),vt,T,r,kap*th,0.5,kap+lda-rho*sig,rho,sig,phi);
P1 = 0.5+(1/pi)*sum(real(exp(-i*phi*log(K)).*f1./(i*phi))*dphi);
f2 = CF_Heston(log(St),vt,T,r,kap*th,-0.5,kap+lda,rho,sig,phi);
